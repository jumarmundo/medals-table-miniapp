...
export class CountryRepository implements ICountryRepository {
  async fetchAll(): Promise<Country[]> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/data/medals.json`, { cache: "no-store" });
    ...
    return (arr as any[]).map(
      (o) => new Country(o.code, Number(o.gold), Number(o.silver), Number(o.bronze))
    );
  }
}
